...
if isempty(ref)
			%% Yand add codes here to consider situation that there is no pv
			if isempty(pv)
				ref = [];
				return;
			end
...
%%---------------------------------%%
% Yang add codes here to consider the generator with largest capacity as
% ref
pvgen = find(ismember(gen(:,GEN_BUS), pv));
maxgen = pvgen(gen(pvgen,PMAX) == max(gen(pvgen,PMAX)));
ind = find(pv == gen(maxgen(1),GEN_BUS));
ref = pv(ind);
pv = pv(~ismember(pv,ref));
% END
%-------------------------------------------------------------%%
end
