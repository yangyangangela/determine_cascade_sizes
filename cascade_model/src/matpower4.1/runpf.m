...
%   YANG MODIFIED THIS WORK ON AUG. 2013 TO CONSIDER THE ISLANDING CASE.
%  If the network is broken into several pieces. And a reference (PV bus)
%  to each piece, if there is no PV bus, set all these piece status to
%  "off".
...
%%-----------------------------------------%%
%% YANG ADD LINES TO SET ISOLATED BUS ZONE = 0

% set isolated buses as loss zone 0
mpc.bus(find(mpc.bus(:,BUS_TYPE)==4),ZONE) = 0;
%% END
%%-----------------------------------------%%
					   
%%-----------------------------------------%%
%% YANG REPLACES THESE TWO LINES HERE
%% convert to internal indexing
tempmpc = ext2int(mpc);
[baseMVA, bus, gen, branch] = deal(tempmpc.baseMVA, tempmpc.bus, tempmpc.gen, tempmpc.branch);
% 
% %% convert to internal indexing
% mpc = ext2int(mpc);
...
%%---------------------------------------------------%%
%% YANG ADD FEW LINES HERE TO REMOVE THE ISOLATED ISLAND WITHOUT GENERATORS
%%
%% build B matrices and phase shift injections
[B, Bf, Pbusinj, Pfinj] = makeBdc(baseMVA, bus, branch);
%% find components in each B matrix
[ci sizes] = components(B);

%% add a slack bus to each component
for i = 1 : max(ci)
    % find the bus index and gen index in sub-network
    cind = find(ci==i);
    n2i = sparse(bus(cind,BUS_I),ones(length(cind),1),cind,...
        max(bus(:,BUS_I)), 1);
    
    tempgind = n2i(gen(:,GEN_BUS));
    gind = find(tempgind);
    
    % reorder the bus index and gen index
    tempn2i = sparse(bus(cind,BUS_I),ones(length(cind),1),1:length(cind),...
        max(bus(:,BUS_I)), 1);
    tempbus = bus(cind,:);
    tempgen = gen(gind,:);
    tempgen(:,GEN_BUS) = tempn2i(tempgen(:,GEN_BUS));
    [tempref, temppv, temppq] = bustypes(tempbus,tempgen);
    
	% set buses zone as the component index
	tempmpc.bus(cind,ZONE) = i;
					   
    % if there is no pv bus, so we don't add new refs
    if isempty(tempref)
        tempmpc.bus(cind,BUS_TYPE) =4; % set these buses as isolated
		tempmpc.bus(cind,ZONE) = 0;	% set isolated buses as loss zone 0
    end
end

%% convert to internal indexing again
mpc = ext2int(int2ext(tempmpc));
[baseMVA, bus, gen, branch] = deal(mpc.baseMVA, mpc.bus, mpc.gen, mpc.branch);
%% get bus index lists of each type of bus
[ref, pv, pq] = bustypes(bus, gen);
...
if dc                               %% DC formulation
    ...
    %%---------------------------------------------------%%
    %% YANG ADD FEW LINES HERE TO ADD REF
    %%
    ...
    for i = 1 : max(ci)
        ...
        % add new reference buses to queque
        ref = [ref;cind(tempref)];
        pv(pv==cind(tempref)) = [];
        Va(cind) = dcpf(B(cind,cind), Pbus(cind), Va0(cind), tempref, temppv, temppq);
    end
    %update reference buses info
...
%     Va = dcpf(B, Pbus, Va0, ref, pv, pq);
    ...
    for k = 1:length(ref)
        %%----------------------------------------%%
        %% Yang replaced these two lines here because in 
        % bustype, Yang set the generator with larges Pmax as reference
        temp = find(gbus == ref(k));
        ind = find( gen(temp,PMAX) == max(gen(temp,PMAX)));
        refgen(k) = on(temp(ind(1)));
